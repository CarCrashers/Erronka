import React from 'react';
import Layout from '../components/layout/layout.jsx';
import ReturnBack from '../components/ui/buttons/returnBack/returnBack.jsx';
import Carrusel from '../components/ui/carrusel/carrusel.jsx';
import Card from '../components/ui/cards/kotxeak/kotxeak.jsx';
import Goikoa from '../components/ui/goikoa/goikoa.jsx';


function Details() {
  return (
    <React.StrictMode>
      <Layout>
        <Goikoa>
            <h1>Produktuaren informazioa</h1>
            <p>Behar duzun kotxea edo pieza aurkitu</p>
        </Goikoa>

        <div className='container-fluid'>
            <div className='row'>
                <div className='col-12'>
                    <ReturnBack text='Katalogora itzuli' href='/erosi' />
                </div>
            </div>

            <div className='row g-3 g-md-4'>
                <div className='col-12 col-lg-6'>
                    <Carrusel />
                </div>

                <div className="col-12 col-md-6 col-lg-3">
                    <div className="d-flex flex-column p-3 rounded-4 border shadow-lg h-100">
                        <h3 className="h5 h-md-4">Produktuaren izena</h3>
                        <hr />
                        <h3 className="h5 h-md-4">Marka</h3>
                        <ul className="py-3 ps-3 small">
                            <li>Datuak: Datu</li>
                            <li>Datuak: Datu</li>
                            <li>Datuak: Datu</li>
                            <li>Datuak: Datu</li>
                        </ul>
                        <p className="mb-0 small">
                            Descripcion: Lorem ipsum dolor sit amet consectetur adipiscing elit, euismod mauris ridiculus accumsan fames aptent, est venenatis sollicitudin aenean lectus neque.
                        </p>
                    </div>
                </div>

                <div className="col-12 col-md-6 col-lg-3">
                    <div className="d-flex flex-column justify-content-between h-100 p-3 rounded-4 border shadow-lg">
                        <div>
                            <h5 className="text-danger">XX.XX $</h5>
                            <p className="text-success mb-0 small">Stock-a: XX</p>
                        </div>
                        <div className="mt-3">
                            <select className="form-select form-select-sm mb-2">
                                <option>Kopurua...</option>
                                <option>1</option>
                                <option>2</option>
                                <option>3</option>
                                <option>...</option>
                            </select>
                            <button className="bg-orange w-100 rounded-5 border-0 py-2 text-white btn btn-sm">
                                Saskira gehitu
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            
            <div className="row flex-column mt-5 mb-4 p-3 rounded-4 border shadow-lg">
                <div className="col-12 px-3">
                    <h1 className="h4 h-md-3">Antzeko produktuak</h1>
                </div>

                <div className="col-12 d-flex justify-content-between justify-content-md-around flex-wrap mt-3 gap-3">
                    
                </div>
            </div>
        </div>
      </Layout>
    </React.StrictMode>
  );
}

export default Details;
